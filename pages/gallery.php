 ...
 *   1. Real uploaded images from /public/uploads/ (via upload_handler.php)
 *   2. Demo placeholder items (picsum) when no uploads exist yet
 */

require_once ROOT_PATH . '/includes/upload_handler.php';

// ── Build gallery items from uploads or demo data ────────────────────────────

$uploaded   = getUploadedImages();
$hasUploads = !empty($uploaded);

if ($hasUploads) {
    // Map uploaded images to the standard $galleryItems structure
    $galleryItems = array_map(fn($img) => [
        'label' => $img['caption'] ?: date('d/m/Y', $img['mtime']),
        'type'  => $img['type']    ?: 'ทั้งหมด',
        'desc'  => $img['caption'] ?: $img['filename'],
        'img'   => $img['url'],
    ], $uploaded);
} else {
    // Demo data – shown until real images are uploaded
    $galleryItems = [
...
// Collect unique types present in this dataset for filter buttons
$presentTypes = array_unique(array_column($galleryItems, 'type'));
sort($presentTypes);
$filterTypes = array_merge(['ทั้งหมด'], $presentTypes);
?>
...
<section class="relative py-20 md:py-28 overflow-hidden" style="background-color: var(--bg-secondary);">
  ...
  <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
    ...
    <?php if (!$hasUploads): ?>
      <!-- Demo notice - only shown when no real images uploaded yet -->
      <p class="mt-4 text-xs text-gray-600 italic">
        (ตัวอย่าง – <a href="/upload" class="text-orange-500/60 hover:text-orange-400 underline underline-offset-2">อัปโหลดรูปจริง</a>เพื่อแทนที่)
      </p>
    <?php else: ?>
      <p class="mt-4 text-xs text-gray-600">
        <a href="/upload" class="text-orange-500/60 hover:text-orange-400 transition-colors">+ เพิ่มรูปภาพ</a>
      </p>
    <?php endif; ?>
  </div>
</section>
...
<section class="py-16 md:py-24" style="background-color: var(--bg-primary);">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <!-- Filter buttons (built from data, not hardcoded) -->
    <div class="flex flex-wrap gap-3 justify-center mb-12 fade-in">
      <?php foreach ($filterTypes as $type): ?>
        <button
          ...
          data-filter="<?= htmlspecialchars($type) ?>">
      ...
      <?php foreach ($galleryItems as $i => $item): ?>
        <div class="gallery-item fade-in rounded-sm overflow-hidden aspect-[4/3] relative cursor-pointer bg-[#1e1e1e]"
          ...
          <div class="absolute bottom-0 left-0 right-0 px-3 py-2"
            ...
            <?php if ($item['type'] && $item['type'] !== 'ทั้งหมด'): ?>
              <span class="bg-orange-500 text-white text-xs font-bold px-2 py-0.5 mr-1.5">
...
function openGalleryItem(el) {
  ...
  if (!lightboxImg || !lightboxEl) return;
  lightboxImg.src    = img;
